...
function initCountdown() {
  ...
  document.getElementById("duyenBirthday").textContent = formatDateDisplay(
    importantDates.duyenBirthday
  );
  document.getElementById("bangBirthday").textContent = formatDateDisplay(
    importantDates.bangBirthday
  );
  ...
  if (timeDiff > 0) {
    ...
    if (eventName.includes("Birthday")) {
      ...
      document.getElementById(
        `${eventName === "duyenBirthday" ? "duyen" : "bang"}Birthday`
      ).textContent = formatDateDisplay(nextBirthday);
    }
  }
}
