...
function loadMemories() {
  // Try to load from localStorage first (for latest changes)
  const localData = localStorage.getItem("memoriesData");
  if (localData) {
    try {
      memories = JSON.parse(localData);
      displayMemories();
      return;
    } catch (error) {
  ...
  // Load memories from JSON file as fallback
  fetch("data/memories.json")
    ...
    .catch((error) => {
      ...
      // Fallback to sample data only if no data exists
...
// Helper functions for saving memories
function getNextMemoryId() {
  if (memories.length === 0) return 1;
  return Math.max(...memories.map(m => m.id || 0)) + 1;
}
...
function saveMemoriesToStorage() {
  ...
  // Auto-generate download file for updating server data
  downloadUpdatedJSON();
}

function resetFormAndUI(form) {
  // Reset form
  form.reset();
...
function saveMemory(e) {
  ...
  if (showImages) {
    ...
    images = Array.from(imageElements).map(img => {
      // Extract filename from src if it's a data URL or full path
      const src = img.src;
      if (src.includes('data/images/')) {
        return src.split('data/images/')[1];
      }
      return src;
    });
  }
  ...
  const memoryData = {
    id: editingId ? parseInt(editingId) : getNextMemoryId(),
    title,
    date,
    category,
    content,
  ...
  if (editingId) {
    ...
    // Add new memory to the end of array (không tạo mới hoàn toàn)
    memories.push(memoryData);
    showNotification('Thêm kỷ niệm thành công!', 'success');
  }
  
  // Save to localStorage and generate download file
...
function downloadUpdatedJSON() {
  ...
  const jsonData = {
    memories: memories, // Đảm bảo dữ liệu được thêm vào cuối mảng existing memories
    couple_info: {
      boy: {
        name: "Phan Văn Bằng",
  ...
  showNotification('Đã tải xuống file memories.json cập nhật! Thay thế file trong thư mục data/ để áp dụng thay đổi.', 'info');
}
